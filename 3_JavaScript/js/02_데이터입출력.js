...
// innnerHTML로 읽어오기 (요소 내부 내용(태그, 속성, 내용)을 얻어와 문자열로 반환)
function getInnerHTML1() {

    // HTML 문서 내에서 id 속성값이 "test1"인 요소를 얻어와
    // 상수형 변수 t1에 저장
                //__HTML 문서 전체를 객체로 볼 때 DOM

    const t1 = document.getElementById("test1");  //__JS는 따로 기본형이 없음.

    // 브라우저 콘솔에 출력 == java의 println
...
function add(){
    ...
    area1.innerHTML += "<div class='area1-box'></div>"; //_ += 이전 박스가 사라지지 않고 계속 누적.
}
...
function getInnerText1(){
    // clg 자동 완성 -> console.log()   (단, JS ES6 snippet 확장파일 필요)
...
function setInnerText1(){
    t2.innerText = "<p class='red'> 변경된 내용 입니다.</p>";  //_태그가 적용이 안됨.
}
...
// window.prompt("내용")
function fnPrompt(){  //_버튼의 값이 입력한 값으로 바뀌게.
    const input = prompt("변경할 버튼명을 입력하세요.");
                // 입력한 값 or null 이 input에 저장
