...
function fnTest4(){
    ...
    // input 요소에 ""(빈 문자열) 대입  == 값을 삭제  //__input태그의 초기값설정
    input.value = ""; 
}
...
function fnTest6(){
    ...
    //_요소를 상수에 저장하는 이유 : 변하지 않을 값이라는 것을 알아서. 코드 꼬임 방지.
...
function inputEnter(){
    ...
    // keydown : 모든 키가 눌렸을 때  _모든 키를 인식함.    
    // keypress : 영어/숫자/특수문자/띄어쓰기가 눌러지고 있을 때.  _이것만 인식함.
    // -> 꾹 누르고 있을 경우, 연속적으로 함수가 호출되는 문제 발생  _ 계속 인식됨.
    // --> keyup 이용(1회만 인식)
...
function readValue(){ // 입력된 값을 읽어들여 채팅창에 출력
    ...
    // 입력한 내용 삭제  _정상적으로 입력이 됐든 안됐든 입력창은 모두 지우도록.
    input.value = ""; 
