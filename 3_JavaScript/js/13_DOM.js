...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    // 요소.childNodes[인덱스]: 인덱스 번째 자식 노드를 모두 반환(NodeList 형태)
    ...
    // #li1의 부모의 배경색을 orange로 변경 (노트 탐색 활용 방법)
...
document.getElementById("btn2").addEventListener("click", function(){
    ...
    console.log(li1.parentElement.parentElement.parentElement.parentElement);  
                //_ body - html - null(문서 내 한정이라 document까지 올라가지는 않음)


    // #test의 모든 자식 요소 탐색  
    const list = test.children; // HTMLCollection 형태로 반환 _ != NodeList. 노드가 아닌 요소이기 때문.
    console.log(list);
    ...
    list[2].previousElementSibling.addEventListener("click", function(){
        ...
        // 이벤트가 발생한 요소의 다음 형제 요소의 배경색을 red로 변경
...
btn3.addEventListener("click", function(){
    if(count3 <= 10){
        ...
        count3++;  // 1증가
    }  //_ 요소가 더해지는 것이 아니라 새로 만들어져서 대신함. (특징이자 단점)
})


// createElement     //_내용을 덮어씌우지 않고 남아있는 상태에서 추가됨.
...
btn4.addEventListener("click", function(){
    ...
    if(count4 <= 10){
        ...
        //  -> 해당 태그 요소를 생성하여 반환
        //  -> 요소가 생성될 뿐이지 화면에 바로 추가되는 것이 아니다!!
